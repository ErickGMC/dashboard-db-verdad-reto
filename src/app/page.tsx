  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      if (highestSimilarity > 0.80) {
        // Mayor al 80%: Eliminar automáticamente
        showModal(
          "Idea Descartada Automáticamente", 
          `Se eliminó automáticamente por tener una similitud del ${(highestSimilarity * 100).toFixed(1)}% (supera el 80%).\n\nPregunta existente:\n"${duplicateText}"`, 
          "error"
        );
        setResults(prev => prev.filter(r => r.id !== id));
        return; 
      } else {
        // Hasta 80%: Mostrar al usuario para que decida
